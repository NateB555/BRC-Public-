"""
BRC Dash App - Public Version

This Dash app simulates the impact of adding rooftop solar PV and battery storage 
on electricity consumption, cost, and GHG emissions in Irish households.

Author: NateB555
Source: https://github.com/NateB555/BRC-Public-
...
#!# 2024 data not currently used by the code, but if you want to use it, there might be an issue with leap years - might need to edit process_to_single_year function #!#

GHG_intensities_2023 = pd.read_csv("eirgrid_ghg_intensity_2023.csv")
GHG_intensities_2024 = pd.read_csv("eirgrid_ghg_intensity_2024.csv")
...
#!# Can be changed for different battery chemistries #!#
...
def G_2_B(SOC_kWh, charge_threshold_kWh, max_charge_discharge_rate, slow_charge_on, slow_charge_rate, 
    ...
    '''Grid-to-Battery. Returns by how much the battery should be charged from the grid'''
...
def G_2_H(consumption, B2H, S2H):
    '''Grid-to-Home. Returns total grid import required to meet consumption.'''
...
def S_2_H(consumption, solar_gen, max_inverter_ac_power):
    '''Solar-to-Home. Returns by how much solar generation should be used to reduce consumption'''
    S2H = min(solar_gen,
              consumption,
...
def B_2_H(consumption, SOC_kWh, S2H, discharge_threshold_kWh, max_charge_discharge_rate,
          battery_efficiency=0.9):
    '''Battery-to-Home. Returns by how much the battery should be discharged to the home.'''
...
def S_2_B(consumption, solar_gen, SOC_kWh, charge_threshold_kWh, max_charge_discharge_rate, 
          battery_efficiency=0.9):
    '''Solar-to-Battery. Returns by how much the battery should be charged from excess solar'''
    
    if batterychargeable(SOC_kWh, charge_threshold_kWh) == True:
        charge_amount_kWh = min(max_charge_discharge_rate*battery_efficiency,
                                abs(consumption-solar_gen)*battery_efficiency,
                                charge_threshold_kWh - SOC_kWh
                                )
    else: charge_amount_kWh = 0.0
...
def S_2_G(consumption, solar_gen, max_inverter_ac_power, 
    ...
    '''Solar-to-Grid. Returns by how much excess solar should be exported to the grid'''
    ...
    children=[
        ...
        html.Div(
            ...
            children=[
                ...
                html.Br(),
                html.H4("COMPARISON TO BASE CASE", style={'textAlign': 'center'}),
    ...
    if full_retrofit_output is not None:
        try:
            ...
            fig = px.pie(
                ...
                names='Endpoint',
                values='Total',
                title=f'Consumption proportion by source (Grid, Solar)<br><sub>Self-Consumption Ratio (SCR): <b>{SCR_percentage:.2f} %</b></sub>',
            )
